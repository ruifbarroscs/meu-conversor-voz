...
# Configuração da Voz de Portugal
...
# Interface
st.title("🎙️ Repetidor de Texto PT-PT")
set_bg()

# Recupera texto salvo na sessão ou inicia vazio
...
if iniciar:
    ...
    status_placeholder.success("Ciclo Ativo! Mantenha esta página aberta.")
    
    while True:
        # 1. Gera e toca o áudio
        ...
        for i in range(20 * 60, 0, -1):
            ...
            status_placeholder.info(f"Próxima leitura em: {mins:02d}:{secs:02d}")
            time.sleep(1)
            if parar: break
        
        if parar:
            status_placeholder.warning("Repetição interrompida.")
            break
